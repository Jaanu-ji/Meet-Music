...
import { ArrowLeft, Search, Sparkles, Zap, Heart, Activity, MapPin } from 'lucide-react-native';
import { artistsApi } from '../lib/api';
import { initialsFromName, resolveProfileImage, withImageCacheBust } from '../lib/profileImage';
import type { RootStackParamList } from '../../App';

type Nav = NativeStackNavigationProp<RootStackParamList, 'DanceHome'>;
...
const POPULAR_STYLES = ['Bharatnatyam', 'Kathak', 'Bollywood', 'Hip Hop', 'Odissi', 'Contemporary'];
const ALL_STYLES = [
  'Bharatnatyam', 'Kathak', 'Odissi', 'Kuchipudi',
  'Manipuri', 'Classical Dance', 'Bollywood', 'Hip Hop',
  'Contemporary', 'Jazz', 'Ballet', 'Folk Dance',
];

const iconForStyle = (s: string) => {
  if (['Bharatnatyam', 'Odissi', 'Classical Dance'].includes(s)) return Sparkles;
  if (['Kathak', 'Ballet'].includes(s)) return Heart;
  if (['Bollywood', 'Folk Dance'].includes(s)) return Zap;
  return Activity;
};

export function DanceHomeScreen() {
  ...
  useEffect(() => {
    artistsApi.list({ artistType: 'dance' }).then((res) => {
      const data = res?.data ?? (Array.isArray(res) ? res : []);
      setArtists(data);
    }).catch((e: any) => setError(e?.message || 'Unable to load')).finally(() => setLoading(false));
  }, []);

  const teachersFor = (style: string) =>
    artists.filter(a => (a.services ?? []).some(s => s.toLowerCase() === style.toLowerCase()));

  const danceTeachers = artists.filter(a =>
    (a.services ?? []).some(s => ALL_STYLES.some(st => st.toLowerCase() === s.toLowerCase()))
  );

  const filteredStyles = ALL_STYLES.filter(s => s.toLowerCase().includes(search.toLowerCase()));
    ...
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        <Animated.View entering={FadeInLeft.duration(400)} style={styles.headerRow}>
          ...
          <View>
            <Text style={styles.headerTitle}>Learn Dance</Text>
            <Text style={styles.headerSub}>Discover teachers and dance styles</Text>
          </View>
        </Animated.View>
      </View>
        ...
        {loading ? (
          ...
          <>
            <Animated.View entering={FadeInDown.delay(100).duration(400)}>
              ...
              <View style={styles.grid2}>
                {POPULAR_STYLES.map((style) => {
                  ...
                  const count = teachersFor(style).length;
                  return (
                    <TouchableOpacity key={style} style={styles.styleCard} activeOpacity={0.8} onPress={() => navigation.navigate('CoursesList', { instrument: style, isDance: true })}>
                      <View style={styles.styleIconBox}>
                        <Icon color="#fff" size={22} />
                      </View>
                      <Text style={styles.styleName}>{style}</Text>
                      <Text style={styles.styleCount}>{count} teachers</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
            ...
            <Animated.View entering={FadeInDown.delay(200).duration(400)}>
              ...
              {danceTeachers.length === 0 ? (
                <View style={styles.emptyBox}>
                  <Text style={styles.emptyEmoji}>💃</Text>
                  <Text style={styles.emptyText}>No dance teachers yet. Check back soon!</Text>
                </View>
              ) : danceTeachers.slice(0, 4).map((teacher) => {
            ...
            <Animated.View entering={FadeInDown.delay(300).duration(400)}>
              <Text style={styles.sectionTitle}>All Dance Styles</Text>
              <View style={styles.grid3}>
                {filteredStyles.map((style) => {
                  const Icon = iconForStyle(style);
                  const count = teachersFor(style).length;
                  return (
                    <TouchableOpacity key={style} style={styles.smallCard} activeOpacity={0.8} onPress={() => navigation.navigate('CoursesList', { instrument: style, isDance: true })}>
                      <View style={styles.smallIconBox}>
                        <Icon color="#ec4899" size={18} />
                      </View>
                      <Text style={styles.smallName} numberOfLines={1}>{style}</Text>
...
const styles = StyleSheet.create({
  ...
  styleCard: { width: '47%', backgroundColor: '#1a1a1a', borderWidth: 1, borderColor: '#2a2a2a', borderRadius: 20, padding: 16 },
  styleIconBox: { width: 44, height: 44, borderRadius: 12, backgroundColor: '#ec4899', alignItems: 'center', justifyContent: 'center', marginBottom: 10 },
  styleName: { fontSize: 15, fontWeight: '700', color: '#fff', marginBottom: 4 },
  styleCount: { fontSize: 12, color: '#888' },
  ...
  teacherAvatar: { width: 56, height: 56, borderRadius: 16, backgroundColor: '#ec4899', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 },
  teacherAvatarImg: { width: 56, height: 56 },
  teacherAvatarText: { fontSize: 24 },
  teacherInfo: { flex: 1 },
  teacherName: { fontSize: 15, fontWeight: '700', color: '#fff', marginBottom: 4 },
  tagRow: { flexDirection: 'row', gap: 6, marginBottom: 4 },
  tag: { backgroundColor: 'rgba(236,72,153,0.15)', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 2 },
  tagText: { color: '#ec4899', fontSize: 11, fontWeight: '600' },
  ...
  smallIconBox: { width: 36, height: 36, borderRadius: 10, backgroundColor: 'rgba(236,72,153,0.15)', alignItems: 'center', justifyContent: 'center', marginBottom: 6 },
